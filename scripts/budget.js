...
let annualSalarySummaryDisplay, netSalaryDisplay, totalExpensesDisplay, remainingBudgetDisplay;
...
let exportJsonBtn, importJsonBtn, importJsonInput, clearAllDataBtn, printBtn;
...
const data = {
    ...
    taxes: { federal: 0, state: 0, custom: [] },
    preTaxDeductions: { '401k-trad': 0, custom: [] },
    postTaxDeductions: { medical: 0, custom: [] },
    expenses: { rent: 0, utilities: 0, custom: [] },
    budgetRule: '50-30-20',
    currentLanguage: 'ko',
    isDarkMode: false
};
...
const ITEM_CATEGORIES = {
    'tax-federal-1': 'fixed', 'tax-state-1': 'fixed',
    'deduct-medical-1': 'needs',
    'deduct-401k-trad-1': 'savings',
    'exp-rent-1': 'needs', 'exp-utilities-1': 'needs',
};
...
const translations = {
    en: {
        ...
        label_federal_withholding: 'Federal',
        label_state_tax: 'State',
...
function calculateBudget() {
    ...
    const { needs, wants, savings } = categorizeAll(false);
    const totalAnnualExpenses = needs + wants + savings;
    const remainingBudget = annualNetIncome - totalAnnualExpenses;
    
    return { annualNetIncome, totalAnnualExpenses, remainingBudget };
}
...
function updateDisplay() {
    ...
    Object.assign(data.taxes, { federal: parseFloat(taxInputs.federal.value) || 0, state: parseFloat(taxInputs.state.value) || 0 });
    Object.assign(data.preTaxDeductions, { '401k-trad': parseFloat(preTaxDeductInputs['401k-trad'].value) || 0 });
    Object.assign(data.postTaxDeductions, { medical: parseFloat(postTaxDeductInputs.medical.value) || 0 });
    Object.assign(data.expenses, { rent: parseFloat(expenseInputs.rent.value) || 0, utilities: parseFloat(expenseInputs.utilities.value) || 0 });
    ...
    netSalaryDisplay.textContent = formatCurrency(convertFromAnnual(annualNetIncome, summaryFreq));
    totalExpensesDisplay.textContent = formatCurrency(convertFromAnnual(totalAnnualExpenses, summaryFreq));
    remainingBudgetDisplay.textContent = formatCurrency(convertFromAnnual(remainingBudget, summaryFreq));
    remainingBudgetDisplay.className = remainingBudget >= 0 ? 'positive' : 'negative';

    const annualSummaryP = annualSalarySummaryDisplay.parentElement;
    if (data.calculationMode === 'advanced') {
        const annualGrossSalary = convertToAnnual(data.grossSalary, data.salaryFrequency) + data.customIncomes.reduce((sum, income) => sum + convertToAnnual(income.amount, income.frequency), 0);
...
function renderCustomList(listElement, customItems, type) {
    ...
    listElement.innerHTML = customItems.map((item, index) => {
        ...
        const frequencyOptions = (type === 'income' || type === 'tax' || type === 'pre-tax' || type === 'post-tax' || type === 'expense') ?
...
function populateUiFromData() {
    ...
    netIncomeInput.value = data.netIncome;
...
function updateCharts() {
    ...
    expensesChartInstance = createOrUpdateChart(expensesChartInstance, 'expenses-chart', t.section_expenses_title, 
        [expenseTotals.needs, expenseTotals.wants, expenseTotals.savings], [t.tag_needs, t.tag_wants, t.tag_savings]);

    const overallExpenses = expenseTotals.needs + expenseTotals.wants + expenseTotals.savings;
    budgetDistributionChartInstance = createOrUpdateChart(budgetDistributionChartInstance, 'budget-distribution-chart', t.section_summary_title, 
        [overallExpenses, remainingBudget > 0 ? remainingBudget : 0], [t.label_total_expenses, t.label_remaining_budget]);
...
function categorizeAll(getItems = false) {
    let totals = { needs: 0, wants: 0, savings: 0 };
    let items = { needs: [], wants: [], savings: [] };

    const processItems = (itemData, category, freq) => {
        const amount = convertToAnnual(itemData.amount, itemData.frequency || freq);
        if (amount > 0) {
            totals[category] += amount;
            if (getItems) items[category].push({ name: itemData.name, amount });
        }
    };
    
    // Process standard expenses
    for(const key in expenseInputs) {
        const category = ITEM_CATEGORIES[`exp-${key}-1`] || 'wants';
        processItems({ name: key, amount: data.expenses[key] }, category, data.frequencies.expense);
    }
    // Process custom expenses
    data.expenses.custom.forEach(item => processItems(item, item.category));
    
    if (data.calculationMode === 'advanced') {
        // Process savings from deductions
        for(const key in preTaxDeductInputs) {
            const category = ITEM_CATEGORIES[`deduct-${key}-1`] || 'wants';
            processItems({name: key, amount: data.preTaxDeductions[key]}, category, data.frequencies.preTax);
        }
        data.preTaxDeductions.custom.forEach(item => processItems(item, item.category));
        
        for(const key in postTaxDeductInputs) {
            const category = ITEM_CATEGORIES[`deduct-${key}-1`] || 'wants';
             processItems({name: key, amount: data.postTaxDeductions[key]}, category, data.frequencies.postTax);
        }
        data.postTaxDeductions.custom.forEach(item => processItems(item, item.category));
    }
...
function applyBudgetRule() {
    ...
    let breakdown = [
        ...
        { id: 'savings', label: t.rule_category_savings_debt, goal: annualNetIncome * 0.2, actual: totals.savings }
    ];
    return breakdown;
}
...
function generateLocalAiReport() {
    ...
    try {
        ...
        const savingsRate = Math.round((totals.savings / annualNetIncome) * 100) || 0;
...
async function generateAiReport() {
    ...
    // This is a placeholder for a real API call. It will always fall back to the local report.
    try {
        if (false) { // Fails deliberately
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    Object.assign(taxInputs, { federal: document.getElementById('tax-federal-1'), state: document.getElementById('tax-state-1') });
    Object.assign(preTaxDeductInputs, { '401k-trad': document.getElementById('deduct-401k-trad-1') });
    Object.assign(postTaxDeductInputs, { medical: document.getElementById('deduct-medical-1') });
    Object.assign(expenseInputs, { rent: document.getElementById('exp-rent-1'), utilities: document.getElementById('exp-utilities-1') });
